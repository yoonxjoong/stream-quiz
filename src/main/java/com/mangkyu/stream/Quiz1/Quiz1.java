...
public class Quiz1 {
    ...
    public Map<String, Integer> quiz1() throws IOException {
        ...
        return csvLines.stream().map(line -> line[1].replaceAll("\\s", ""))
                .flatMap(hobbies -> Arrays.stream(hobbies.split(":"))).collect(
                        Collectors.toMap(hobby -> hobby, hobby -> 1,
    ...
    private int countContains(String src, int fromIndex) {
        ...
        if(index > 0 ) {
            return 1 + countContains(src, index + 2);
        }
    ...
    private List<String[]> readCsvLines() throws IOException {
        CSVReader csvReader = new CSVReader(
                new FileReader(getClass().getResource("/user.csv").getFile()));
        csvReader.readNext();
